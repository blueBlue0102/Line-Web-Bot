...
class Client():
    ...
    def loadSession(self):
        ...
        if not all(credKey in credential for credKey in ["authToken", "mid", "userId"]):
    ...
    def getContactList(self):
        return json.loads(self.session.get(
            url='https://chat.line.biz/api/v1/bots/' + self.mid +
            '/contacts?query=&sortKey=DISPLAY_NAME&sortOrder=ASC&excludeSpam=true&limit=100',
            headers=self.defaultHeaders, data=None, allow_redirects=True).text)["list"]
    ...
    def sendSticker(self, chatId, packageId, stickerId):
        data = {"stickerId": stickerId, "packageId": packageId, "type": "sticker", "sendId": chatId + "_" +
                str(int(time.time())) + "_" + ''.join(random.choice(string.digits) for i in range(8))}
        return json.loads(self.session.post(
            url='https://chat.line.biz/api/v1/bots/'+self.mid+'/messages/'+chatId+'/send',
            headers=self.defaultHeaders,
            json=data,
    ...
    def sendFileWithPath(self, chatId, path):
        data = {"file": open(path, 'rb'), "sendId": (None, chatId+"_"+str(int(time.time())
                                                                          )+"_"+''.join(random.choice(string.digits) for i in range(8)))}
        return json.loads(self.session.post(
