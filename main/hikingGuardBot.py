...
class HikingGuardBot:
    ...
    async def __startTrip(self, chatId: str, tripId: str, username: str):
        ...
        if tripData is None:
            # 沒找到
            await self.lineClient.sendMessage(chatId, (f"很抱歉，沒有找到對應的行程代碼（{tripId}）\n" "請確認代碼沒有輸入錯誤，或是再試一次"))
            return
        ...
        await self.lineClient.sendMessage(chatId, message)
        ...
        # 更改暱稱
        await self.lineClient.changeNickname(
            chatId,
            (
                f"{username[0: 2]}"
        ...
        # Follow up
        await self.lineClient.addFollowedUp(chatId)

        # 釘選訊息
        await asyncio.sleep(1.5)
        messageEventList = await self.lineClient.getMessages(chatId)
        for messageEvent in messageEventList:
            if (
                ...
                await self.lineClient.pinMessage(chatId, messageEvent["message"]["id"])
                break

    async def __stopTrip(self, chatId: str):
        """
        「結束行程」的處理邏輯
        """
        await self.lineClient.changeNickname(chatId, "")
        await self.lineClient.addResolved(chatId)
        pinnedMessageList = await self.lineClient.getPinnedMessage(chatId)
        for pinnedMessage in pinnedMessageList:
            if (
                ...
                await self.lineClient.unpinMessage(chatId, pinnedMessage["message"]["id"])
                break

    async def scanChatList(self):
        """
        ...
        async def getUsername(chat) -> str:
            if chat["chatType"] == "USER":
                ...
                return (await self.lineClient.getChat(chat["chatId"]))["profile"]["name"]

        chatList = (await self.lineClient.getChatList(folderType="INBOX"))["list"]
        for chat in chatList:
            ...
            if isClientSendingTextMsg(chat):
                if isTripStart(chat):
                    ...
                    await self.__startTrip(chatId, tripId, username)
                    print(f"User [{username}] start a trip.")
                    await asyncio.sleep(0.25)
            elif isGuardSendingTextMsg(chat):
                if isTripStop(chat):
                    username = await getUsername(chat)
                    await self.__stopTrip(chatId)
                    print(f"User [{username}] stop a trip.")
                    await asyncio.sleep(0.25)

    async def sseChatList(self, shutdownSeconds):
        """
        ...
        poll = await self.lineClient.openPolling()
        startTime = time.time()
                    ...
                    if "data:{" in decodedData:
                        ...
                        if isClientSendingTextMsg(chunk):
                            ...
                            if isTripStart(chunk):
                                ...
                                username = (await self.lineClient.getChat(chatId))["profile"]["name"]
                                await self.__startTrip(chatId, tripId, username)
                                print(f"User [{username}] start a trip.")
